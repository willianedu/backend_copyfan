...
export class MaterialController {
    async createMaterial(req: Request, res: Response){
        ...
        const { file,curso_id,turma_periodo,usuario_id } = req.body
        // Passa informações capturadas para o service
        const result = await service.createMaterial({file,curso_id,turma_periodo,usuario_id})
    ...
    async updateMaterial(req: Request, res: Response){
        const { id } = req.params
        const { file,turma_periodo} = req.body
        const result = await service.updateMaterial({id,file,turma_periodo})
        if (result instanceof Error){
